...
| Layer | File | Role |
| --- | --- | --- |
| Evaluator | `prepare.py` | Load Hamiltonians, compute references, measure energy and compiled gate counts. |
| Experiment surface | `train.py` | Build candidate circuits and optimize parameters. This is the main editable file. |
| Harness | `harness.py` | Inspect Hamiltonians, isolate runs, compare against targets, summarize evidence. |
| Protocol | `program.md` | Short operating contract for agents. |
...
1. Read `program.md`.
2. Run `uv run harness.py inspect --problem <problem>`.
3. Open the relevant doc from `docs/`.
4. Add or adjust one candidate in `train.py`.
...
- Do not encode every possible ansatz as `if/else` policy in `harness.py`.
...
Karpathy's `autoresearch` keeps the repo deliberately small: `prepare.py` is not
modified, `train.py` is the agent-editable file, and `program.md` is the
